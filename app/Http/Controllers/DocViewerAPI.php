...
class DocViewerAPI extends Controller
{
    ...
    public function detail(Request $request)
    {
		...
		if (curl_errno($ch)) {
            ...
            // return $output;
            $hocrPieces = explode(";", $output['highlighting'][$id]['hocr'][0]);
            foreach($hocrPieces as $key=>$value)
            {
                $highlightTemp;
                $pos = strrpos($value, "</em>");
                if($pos)
                {
                $value=substr($value, $pos+5 , strrpos($value, "||")-$pos-5);
                $confidence=trim(substr($value,strpos($value, "||")),"|<> ");
                $value=trim (substr($value, 0 , strpos($value, "||")),"<>| ");
                $highlightoutput[]=$confidence." ".$value;
                }
            }
            $responseData = array("imageSrc" => "/" . $id, "type" => "image", "hocr" =>$highlightoutput );
            return $responseData;
        }
  ...
  //       echo $testURl;
   ...
   //      return;
        ...
        // );
        eval("\$clusterOutput = " . $clusterOutput . ";");
        if($graph=="") {
            $graph= array(
                "name"=> "Root",
                "children"=>array())
                ;
            foreach($clusterOutput["clusters"] as $key=>$cluster)
            {
                $graph["children"][]=array("name"=>$cluster["labels"][0],"size"=>sizeof($cluster["docs"]));
            }
        }    
